...
async def list_models():
    global model_args
    model_card = ModelCard(id="Qwen3-4B")
...
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    if len(prev_messages) > 0 and prev_messages[0].role == "system":
        query = prev_messages.pop(0).content + query #prev_messages.pop(0) 的作用是移除 prev_messages 列表中索引位置为 0 的元素，并返回该元素的值。同时，由于该元素是从列表中移除的， prev_messages 列表的长度会减少一个元素。
    
    # 将每次的"user"提问和"assistant"回答打包依次储存为history（由于prev_messages.pop(0)，这里已经没有了system ）
    history = []
    # 用 generate 生成回复
    response_text = chat_with_generate(
        history_messages=[m for pair in history for m in pair],
        query=query,
        max_new_tokens=request.max_length or 256,
...
async def predict(query: str, history: List[List[str]], model_id: str):
    ...
    yield "{}".format(chunk.json(exclude_unset=True, ensure_ascii=False)) # 使用yield进行流式输出
...
if __name__ == "__main__":
    tokenizer = AutoTokenizer.from_pretrained("/data6/zhangenwei/BigModel/Qwen/Qwen3-4B", trust_remote_code=True)
    model = AutoModel.from_pretrained("/data6/zhangenwei/BigModel/Qwen/Qwen3-4B", trust_remote_code=True).cuda()
